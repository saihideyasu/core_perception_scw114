...
private:
    ...
    ros::Subscriber sub_config_, sub_gnss_to_baselink_;
                           ...
                           const autoware_msgs::GnssSurfaceSpeed::ConstPtr& speed_msg)
    {
...

        {
            ...
            tf::Quaternion q_correct = tf::createQuaternionFromYaw(config_.yaw_correct * M_PI / 180.0);
            ...
            if(gmh.get_data_size() > 0)
            {
                ...
                geometry_msgs::PoseStamped prev = gmh.get_pose(0);
                pose gnss_pose_prev;
                gnss_pose_prev.x = prev.pose.position.x;//t2_gnss_prev(0, 3);
                gnss_pose_prev.y = prev.pose.position.y;//t2_gnss_prev(1, 3);
                gnss_pose_prev.z = prev.pose.position.z;//t2_gnss_prev(2, 3);
                geometry_quat_to_rpy(gnss_pose_prev.roll, gnss_pose_prev.pitch, gnss_pose_prev.yaw, prev.pose.orientation);
                //mat_b_gnss.getRPY(gnss_pose_prev.roll, gnss_pose_prev.pitch, gnss_pose_prev.yaw, 1);
                //qua.setRPY(gnss_pose_prev.roll, gnss_pose_prev.pitch, gnss_pose_prev.yaw);
	...
	//if(diff_time == 0) return;
        ...
        str << current.yaw << "," << prev.yaw;
        std_msgs::String tmp;
        tmp.data = str.str();
        tmp_pub_.publish(tmp);
...
public:
    gnss_matching_class(ros::NodeHandle nh_, ros::NodeHandle private_nh_)
    {
        ...
        int node_number;
        if (nh.getParam("node_number", node_number) == false)
        {
        ...
        ss_roll << "groll" << node_number;
        if (nh.getParam(ss_roll.str(), _tf_gpitch) == false)
        {
          std::cout << "ay is not set." << std::endl;
          return;
        }
        std::stringstream ss_pitch;
        ss_pitch << "gpitch" << node_number;
        if (nh.getParam(ss_pitch.str(), _tf_groll) == false)
        {
        ...
        tmp_pub_ = nh.advertise<std_msgs::String>("/RTK_tmp", 10);
